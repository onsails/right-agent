...
- **Debugging agent sessions**: In development, bots run with `--debug`. Two log sources: (1) CC debug logs at `~/.claude/logs/` inside sandbox (`/sandbox/.claude/logs/`) or on host for `--no-sandbox`; (2) stream NDJSON logs at `~/.rightclaw/logs/streams/<session-uuid>.ndjson` on host. When diagnosing agent behavior, read these logs directly instead of guessing from Telegram output.
