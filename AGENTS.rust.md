...
- **External integration tests**: tests requiring a live Agent Sandbox (microVM), Claude Code inside a sandbox, real ffmpeg/Whisper inference, or network model downloads must be `#[ignore]` locally and explicitly invoked by GitHub Actions jobs. Use stable ignore reasons and workspace-filterable test name prefixes together: `ci-msb: ...` with `ci_msb_`, `ci-claude: ...` with `ci_claude_`, `ci-stt: ...` with `ci_stt_`. `crates/right/tests/ci_ignored_contract.rs` enforces this so future packages are not missed.
...
- **Live sandbox tests**: microVM tests boot real sandboxes and must acquire a VM slot before booting (`crates/right-sandbox/tests/common/mod.rs`; cap via `RT_MSB_MAX_CONCURRENT_VMS`, default 2) — probe VMs are GiB-scale and several worktrees may run at once. Never hardcode a sandbox name; derive it with `unique_sandbox_name`. Never leak a VM: acquire it through `SandboxGuard` or destroy it in a drop guard of your own. They need a host with a readable `/dev/kvm`; in CI they run in the opt-in `sandbox` job gated on the `SANDBOX_RUNNER` repository variable.
