...
The `## Response Rules` section also defines the delivery-channel rule:
only the structured reply's `content` field is delivered to Telegram —
assistant text blocks never reach the user, and `content: null` sends
nothing. It also defines the act-over-promise rule: a
...
invocation instead of `CURATOR_SYSTEM_PROMPT`. This is a session-bearing
`claude -p --json-schema <CURATOR_PLAN_SCHEMA> --output-format stream-json
--allowedTools Read` call — no `Bash`, no `skill_learning_start/finish`.
`--mcp-config` is present (session-bearing invariant) even though only the
`Read` tool is granted. The system
...
fields). On success, one `curator_runs` row with `mode = 'report_only'` and
`status = 'proposed'` is appended — no `skill_lifecycle` rows are written, no
