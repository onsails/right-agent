...
## Scheduled jobs that improve themselves

Ask the agent in chat to run work on a schedule, a morning digest, a repo
watcher, a weekly report, and it sets up the job for you. A job states a goal and
reuses the agent's skills for the procedure, so it improves as those skills do:
fix a skill once and every job that uses it picks up the change on its next run.
The one thing that never changes on its own is a job's saved instructions, which
change only when the agent edits them and tells you what it simplified. See
[Scheduled jobs](/docs/scheduled-jobs/) for the full pattern.
