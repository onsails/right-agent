...
- The Telegram Mini App dashboard is now live — open it from any bot via `/mcp` to see agent activity, usage breakdown by model and skill, skill inventory with lifecycle state, and health status. MCP OAuth flows can be started and tracked from the dashboard without issuing Telegram commands.
- Agents can hand off in-flight work to a background continuation; the result arrives as a Telegram reply when complete. Graceful shutdown flushes pending background deliveries so in-progress results are not lost.
- The skill-learning pipeline is rebuilt as a three-stage Haiku prefilter → probe-writer → curator flow with per-agent daily budget gates and a circuit breaker; repeated review failures pause learning automatically until a successful review resets the circuit. `/right-learn-skill` now fires only on explicit user intent — deferred automatic triggering is removed.
- Gateway providers (credentials substituted at the sandbox proxy) now have a full management API: create, rotate, update, and remove providers through the dashboard; sandbox attach/detach is wired automatically.
- Agents can now search their archived conversation history scoped to the current thread (`mcp__right__thread_search`) or the full chat (`mcp__right__chat_search`).
- MCP improvements: loopback servers (`http://localhost:…`) can now be registered for local development; servers can authenticate with multiple simultaneous HTTP headers; MCP OAuth status is tracked per-server and surfaced in the dashboard.
- User-local executables installed by the agent (via npm, pip, etc.) are now reliably on `PATH` before every Claude invocation — `~/.local/bin` is sourced in the sandbox `bashrc` and injected before `claude -p`.
