...
- **Reproduce a sandbox `claude` invocation by hand**: To debug a sandboxed `claude -p` invocation manually (e.g. the learning prefilter or any agent turn), SSH into the agent's OpenShell sandbox and run `claude` with the agent's own token. The SSH config is at `~/.right/run/ssh/<sandbox-name>.ssh-config` and the host alias is `openshell-<sandbox-name>` (the `openshell-` prefix is what `right_openshell::openshell::ssh_host_for_sandbox` produces; sandbox names come from `openshell sandbox list`). The agent's Claude OAuth token lives in its per-agent DB and must be injected as `CLAUDE_CODE_OAUTH_TOKEN` — exactly what the bot does in `build_claude_command` (`crates/bot/src/cc/invocation.rs`):
  ```sh
  SSHCFG=~/.right/run/ssh/test-sandbox-20260516-1649.ssh-config
  TOKEN=$(sqlite3 -readonly ~/.right/agents/agent-b/data.db "SELECT token FROM auth_tokens LIMIT 1;")
  ssh -F "$SSHCFG" openshell-test-sandbox-20260516-1649 \
    "export CLAUDE_CODE_OAUTH_TOKEN='$TOKEN'; cd /sandbox && claude -p --output-format json -- 'hi'"
  ```
  Keep the token in a shell variable only — never echo or print it (project rule: never get secrets into context). Sandbox `claude` may return `Not logged in · Please run /login` after the on-disk token expires (OpenShell #1603); injecting `CLAUDE_CODE_OAUTH_TOKEN` bypasses that for ad-hoc tests.
