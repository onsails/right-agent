...
- **MCP with_instructions()**: When adding, removing, or renaming MCP tools, always update `with_instructions()` in `aggregator.rs` to reflect the current tool set and descriptions.
