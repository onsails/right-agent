...
4. [executing] Background typographic experiment — replace cliché starfield/constellation with a typographic background structure. Build ALL FOUR concepts behind a `BG-EXPERIMENT` switcher to compare: A ghost wordmark · B mono telemetry field · C drifting manifesto · D engineering blueprint. Keep dark jewel base + grain + scan; remove `.stars-*`/`.twinkle`/`.constellation`. spec:04-bg-typo-spec.md plan:04-bg-typo-plan.md mimo:bgtypo-3p7w (openai/gpt-5.4 max, pinned)
5. [todo]    Lock-in chosen background — keep the picked concept, remove the other three + the switcher. Gated on stage 04 pick.
