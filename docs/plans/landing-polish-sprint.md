...
5. [executing] Lock telemetry + paper-texture experiment — user picked B (telemetry); remove A/C/D + the bg switcher, make telemetry unconditional. Add a heavy STATIC paper texture (4 textures × 2 tones behind a new `PAPER-EXPERIMENT` switcher: fiber/cardstock/halftone/crosshatch × dark/warm) so the typography feels printed. spec:05-paper-texture-spec.md plan:05-paper-texture-plan.md mimo:paper-7c2k (openai/gpt-5.4 max, pinned)
6. [todo]    Lock-in chosen texture — keep the picked texture+tone, remove the other textures + the switcher. Gated on stage 05 pick.
