...
7. [done] Texture experiment v3 — bg motion killed (`.scan` sweep + grid `flow` removed → fully static bg, only telemetry flicker remains); added 10 tactile relief textures (kraft/coldpress/letterpress/corrugated/deckle/sandpaper/burlap/slate/leather/cork via feDiffuse/feSpecular/feDisplacement + emboss shadows); expanded `data-tone` to 7 colors (plum/kraft/slate/sepia/olive/wine/ink, all `color-mix` of tokens, no new hexes). 17 textures × 7 colors behind the switcher (default cardstock/plum). spec:07-texture-v3-spec.md plan:07-texture-v3-plan.md (merged @53d82928 · stage @3fd1620a · astro check 0 · build green · grep gates clean · all static · mimo texv3-2q9r openai/gpt-5.4 max, transient OpenAI errors → resumed). **AWAITING USER PICK (texture + color) → feeds stage 08.**
8. [todo]    Lock-in chosen texture+color — keep the picked combo, remove the rest + the switcher. Gated on stage 07 pick.
