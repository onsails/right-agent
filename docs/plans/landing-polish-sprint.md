...
6. [executing] Texture experiment v2 + remove bg parallax — user: bg parallax is dated → remove (bg static; keep section-zoom/reveal/telemetry-flicker); drop `fiber` (invisible on dark); strengthen cardstock/halftone/crosshatch; add 4 heavier textures (concrete/canvas/grunge/speckle). 7 textures × 2 tones behind the existing switcher. spec:06-texture-v2-spec.md plan:06-texture-v2-plan.md mimo:texv2-5h8n (openai/gpt-5.4 max, pinned)
