...
3. [executing] Finalize FX — user rejected all 3 subtle variants → ONE explicit effect: active-section focus (centered `.section` ~1.06 + bright, neighbors ~0.96 + dim ~.55, smooth on scroll, overflow-x guarded); rework `learning` to crisp sequential fill (no shimmer); remove ALL `FX-EXPERIMENT` scaffolding (switcher + A/B/C). Consolidates the old stage-03 lock-in. spec:03-finalize-fx-spec.md plan:03-finalize-fx-plan.md mimo:scrollfx-9m4k (resume, openai/gpt-5.4 max, pinned)
