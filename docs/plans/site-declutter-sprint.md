...
6. [done] Polish & fixes — removed leftover hero `.rev` (hero now static, crisp on first paint); a11y: hero `<header>`→`<section>`, content wrapped in `<main>`, footer stays `contentinfo` (one banner/main/contentinfo); eyebrow+`.label` tracking .24em→.14em; pruned dead CSS (`.status`/`.pdot`/`@keyframes blink`/`.statusnote*`/`.bk*`/`.flab`; `.cmd*` kept). spec:06-polish-spec.md plan:06-polish-plan.md (merged @846960d4 · 3 files · +14/−25 · astro check 0 · build green · review xhigh clean · mimo polish-6f2e venice/openai-gpt-55 high — ran CLEAN this time, no substitution).
7. [done] Reduce icon noise — removed the caption icon from `.how-cap` in FlowStrip + Diagram (every-chat message dup, round-trip arrow dup, credentials key dup all gone); captions now text-only; node/`.credkey`/`.how-back` icons kept; `capIcon` prop fully removed; dropped dead `.how-cap .ic-sm` CSS. spec:07-iconnoise-spec.md plan:07-iconnoise-plan.md (merged @e327f50d · 4 files · +5/−8 · astro check 0 · build green · review clean · mimo icons-4d7b venice errored → hand-applied, diff review+build verified).
8. [done] Monospace headline (#3) — hero `h1` + section `h2` → JetBrains Mono via new `--headline` token (herdr's font; live-previewed, beat Martian Mono which overflowed); tracking -.04/-.03em; de-skewed `.glow` accent word (upright, keeps ruby + teal underline). Wordmark/card/step titles + docs stay Chakra Petch (two-tier hierarchy verified). spec:08-mono-headline-spec.md plan:08-mono-headline-plan.md (merged @771a839d · 2 files · astro check 0 · build green · review clean · venice errored → hand-applied, verified).
