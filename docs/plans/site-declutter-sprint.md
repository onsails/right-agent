...
7. [planned] Reduce icon noise — drop the caption icon (`.how-cap` `.ic-sm`) in FlowStrip + Diagram (every-chat msg dup, round-trip arrow dup, credentials key dup); captions become text-only; node/credkey/how-back icons kept; remove `capIcon` prop + `.how-cap .ic-sm` CSS. spec:07-iconnoise-spec.md plan:07-iconnoise-plan.md wt:.worktrees/07-iconnoise mimo:icons-4d7b
