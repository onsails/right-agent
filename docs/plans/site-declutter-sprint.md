...
8. [planned] Monospace headline (#3) — hero `h1` + section `h2` → JetBrains Mono via new `--headline` token (herdr's font; live-previewed, beat Martian Mono which overflowed); tighten tracking; de-skew `.glow` accent word. Wordmark/card/step titles + docs stay Chakra Petch. spec:08-mono-headline-spec.md plan:08-mono-headline-plan.md wt:.worktrees/08-mono-headline mimo:mono-3c8f
