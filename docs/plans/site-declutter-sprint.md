...
6. [planned] Polish & fixes — (a) remove leftover hero `.rev` → static hero (finishes stage-2 goal); (b) a11y: hero `<header>`→`<section>`, wrap content in `<main>`, footer stays `contentinfo`; (c) tighten eyebrow/`.label` tracking .24em→.14em; (d) prune dead CSS (`.status`/`.pdot`/`@keyframes blink`/`.statusnote`/`.bk`/`.flab` — all 0 .astro refs; KEEP `.cmd*`). spec:06-polish-spec.md plan:06-polish-plan.md wt:.worktrees/06-polish mimo:polish-6f2e
