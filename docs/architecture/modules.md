...
- `Connection`, `Transaction`, `DbError` — project-owned wrappers over the local Turso driver for per-agent SQLite-compatible `data.db`; legacy FTS5 cleanup uses bundled `rusqlite` before writable Turso opens.
