...
**Architecture:** Keep the migration in `right_openshell::providers::reconcile_for_sandbox`, the single gateway provider reconcile path used by bot startup and hot-reconcile. The reconciler repairs only declared providers that already exist and still have the legacy generic type by recreating the gateway provider with the same name and the Right-managed profile type, preserving gateway-held fields returned by `GetProvider`. Bot bring-up keeps the same recoverable degraded retry behavior, but maps provider reconcile/composition failures to a provider-specific `GatewayCause`.
...
**Post-production correction:** Real OpenShell rejects provider type changes through `UpdateProvider` with `provider type cannot be changed; delete and recreate the provider`. The final repair path is therefore `GetProvider` → optional detach → `DeleteProvider` → `CreateProvider` with the new profile type → attach, with best-effort rollback if recreate fails. Earlier task snippets below that assert update-based repair are historical and superseded.
...
- OpenShell `UpdateProvider` with empty credentials/config maps is sparse for existing provider fields, but cannot change provider type. Legacy generic type repair must recreate the provider instead.
...
- Modify `crates/right-openshell/src/providers.rs`: add legacy generic type repair helper, extend `ReconcileReport`, recreate legacy generic providers before attach.
- Modify `crates/right-openshell/src/providers_tests.rs`: add regression tests for legacy `type = "generic"` repair and recreate failure behavior.
