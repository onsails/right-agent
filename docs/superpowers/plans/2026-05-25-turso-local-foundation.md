...
In workspace [Cargo.toml](/Users/molt/dev/rightclaw/Cargo.toml), add the latest registry version from Step 0.3 near the current `libsql` entry:
...
In [crates/right-db/Cargo.toml](/Users/molt/dev/rightclaw/crates/right-db/Cargo.toml), add `turso` while keeping `libsql`:
...
Create [turso_compat.rs](/Users/molt/dev/rightclaw/crates/right-db/tests/turso_compat.rs):
...
In [params.rs](/Users/molt/dev/rightclaw/crates/right-db/src/params.rs), replace all `libsql` references with `turso`, and rename `into_libsql` to `into_turso`.
...
In [row.rs](/Users/molt/dev/rightclaw/crates/right-db/src/row.rs), replace the file with this driver-neutral public shape:
...
In [error.rs](/Users/molt/dev/rightclaw/crates/right-db/src/error.rs), replace `libsql::Error` with `turso::Error` and replace constraint classification with Turso variants:
...
In [connection.rs](/Users/molt/dev/rightclaw/crates/right-db/src/connection.rs):
...
In [connection.rs](/Users/molt/dev/rightclaw/crates/right-db/src/connection.rs), replace each `into_libsql()` call with `into_turso()`, and replace `block_on_libsql` with:
...
In [connection.rs](/Users/molt/dev/rightclaw/crates/right-db/src/connection.rs), replace `transaction_with_behavior` with:
...
In [connection.rs](/Users/molt/dev/rightclaw/crates/right-db/src/connection.rs), keep the busy timeout, and test whether Turso accepts WAL mode:
...
In [transaction.rs](/Users/molt/dev/rightclaw/crates/right-db/src/transaction.rs):
...
In [smoke.rs](/Users/molt/dev/rightclaw/crates/right-db/tests/smoke.rs), change the first import to:
...
In [smoke.rs](/Users/molt/dev/rightclaw/crates/right-db/tests/smoke.rs), rename these tests and messages:
...
In workspace [Cargo.toml](/Users/molt/dev/rightclaw/Cargo.toml), remove:

```toml
libsql = "0.9.30"
```

In [crates/right-db/Cargo.toml](/Users/molt/dev/rightclaw/crates/right-db/Cargo.toml), remove:
...
In [ARCHITECTURE.md](/Users/molt/dev/rightclaw/ARCHITECTURE.md), update the workspace table row:
...
In [docs/architecture/modules.md](/Users/molt/dev/rightclaw/docs/architecture/modules.md), update the `right-db` bullet:
...
In [docs/architecture/memory.md](/Users/molt/dev/rightclaw/docs/architecture/memory.md), replace current local-driver wording with:
