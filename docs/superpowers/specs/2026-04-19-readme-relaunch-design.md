...
> Managed with Hindsight Cloud for semantic recall (append-only), or as a plain `MEMORY.md` file the agent curates itself. Either way, memory survives restarts and compounds over time. Each agent also writes its own identity and personality on first launch. Details in §4.
...
5. **Factual boundaries in §5.** As of 2026-04-19:
   - RightClaw does not generate OpenShell credential-provider config. §5.3 frames providers as a "can go further" capability and ties them to the Next roadmap bullet. Do not strengthen that wording until the wiring lands.
   - RightClaw policies do not use HTTP method+path filters, only domain-level allowlists. §5.2 must stay at the domain-allowlist level. Do not add method/path examples until the policy generator emits them.
   - Claude OAuth token is injected into the sandbox's environment per invocation (`CLAUDE_CODE_OAUTH_TOKEN`). §5.3 must not claim Claude auth "stays on the host" in the same breath as MCP tokens; current wording correctly distinguishes the two.
   - Filesystem policy is hardcoded in codegen (not user-configurable). §5.1 must describe isolation in terms of "the sandbox's own workspace", not "the paths you allowed".
