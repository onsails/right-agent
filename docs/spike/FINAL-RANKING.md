...
## mistral.rs (research 2026-06-19) — a SERVING-layer candidate, NOT a harness

`EricLBuehler/mistral.rs` is a Rust **inference engine** (vLLM/llama.cpp-class): loads weights, runs generation. It is the self-host model-SERVING layer that sits UNDER a harness (rig/Goose/claude), not a competitor to them.
- **Structured output = native constrained decoding** via `llguidance` logit-masking (`mistralrs-core/src/pipeline/llg.rs`, `sampling.rs:649 compute_mask_or_eos`). Requested as OpenAI `response_format:json_schema`, a `grammar` param (Regex/Lark/JSON-Schema/llguidance), or embeddable `model.generate_structured::<T>()`. Enforced in the sampler → **structurally immune to the Venice/mimo `propertyNames` tooling bug** (the engine owns the grammar). Composes with forced tool calls.
- **API: OpenAI `/v1` AND Anthropic `/v1/messages`** (`mistralrs serve`), embeddable Rust crate `mistralrs`, Python. The Anthropic endpoint is what makes Option C (below) proxy-free. A single Rust binary could embed rig+mistralrs in-process (caveat: workspace pins `candle` to a git rev → version-unification fight if the host also uses candle).
- **MCP**: client (`mistralrs-mcp`, Http/SSE/WS/Process) + can expose itself as an MCP server (one `chat` tool). Tool-calling per-arch parsers.
- **Models**: Qwen2/3(+MoE/Next), Llama3.x/4, Mistral/Mixtral, Gemma2/3/4, Phi, DeepSeek V2/V3, **GLM4/GLM4-MoE**, etc. **Kimi/Moonshot NOT listed.** Quant GGUF/GPTQ/AWQ/UQFF/ISQ. CUDA/Metal/CPU — CPU runs but **GPU realistically required** → a separate GPU serving host reached over the network (or Apple-Silicon/CUDA box; or embed the crate).
- **RISK: HIGH operational** — single-maintainer (EricLBuehler ~88% of commits), pre-1.0 (v0.8.17, ~14 releases/90d), open correctness bugs incl. **#2204** (temp=0 + JSON-schema, greedy top token = EOS → deterministic `Parser Error: token doesn't satisfy the grammar`; repro is a tool-routing `oneOf` envelope — the exact RightClaw shape). Validate #2204 on real schemas/temps before betting the structured-output story on it.
- **Fit**: the self-host engine under rig (rig calls its `/v1`), and the piece that makes self-host structured output reliable + RightClaw-controlled (no provider-tooling grammar bugs). Complements the harness; does not change the harness rank.

## OPTION C (2026-06-19, EMPIRICALLY TESTED) — keep `claude -p`, swap `ANTHROPIC_BASE_URL`; AVOIDS the harness migration for multi-model

The user's insight: point Claude Code at a non-Anthropic backend. **Tested with real `claude` 2.1.185 against a fake local Anthropic endpoint** (`docs/spike/harness/fake_anthropic.py`; dummy key, no quota, no secret):
- **`claude -p` honors `ANTHROPIC_BASE_URL`** — completed a turn from the fake endpoint (`result:"PONG"`, `num_turns:1`, `is_error:false`). Wire: `POST /v1/messages?beta=true`, `x-api-key` auth, `anthropic-version:2023-06-01`, UA `claude-cli/2.1.185 (external, sdk-cli)`, system as a block list, **59 built-in tools**, `output_config:{effort:high}`.
- **`--json-schema` is implemented CLIENT-SIDE**: claude injects a synthetic **`StructuredOutput`** tool whose `input_schema` is the verbatim `--json-schema` payload (desc: "You MUST call this tool exactly once at the end"), with **`tool_choice:null` (NOT forced)**. → **The backend needs only ordinary tool-calling; no native Anthropic structured-output API.** (My text-only server never emitted the tool call → claude looped 10 turns → empty result; a real tool-calling model would call it.)
- **mistral.rs serves Anthropic `/v1/messages` natively** → `claude -p` points straight at it, **no translation proxy**. OpenAI-only backends (vLLM/Venice) need LiteLLM / claude-code-router in front.

**What Option C buys:** keep ALL of `claude -p` (skills/subagents/forks/MCP/`--json-schema`/agentic loop + the bot orchestration around it) + add multi-model/self-host via per-agent `ANTHROPIC_BASE_URL`. Near-zero RightClaw code change. Naturally hybrid (Anthropic endpoint for Claude w/ subscription; your backend + dummy key for open models).

**What it does NOT buy — and the decision reframe:** it does **NOT** give harness *control*. The harness stays claude's **closed binary** — the user's original driver («больше контроля над харнессом») is unmet; Option C gives **model** control, not **harness** control (no owning the loop / prompt assembly / learning-pipeline integration / escape from Anthropic changing the binary). And **open-model reliability = the same whitelist problem**, now via an **unforced** `StructuredOutput` tool → weak models may not call it (claude loops → empty, observed); mistral.rs constrained decoding only helps when the tool is *forced*, which claude doesn't do. Plus claude ships 59 tools + Claude-tuned prompting every turn (open models may handle worse).

**Therefore: multi-model/self-host ALONE does not justify migrating** — Option C delivers it cheaply. Migrating to **rig** is justified ONLY by wanting harness CONTROL (own loop, byte-exact prompt, native-not-injected structured output, no closed-binary dependency, deep learning-pipeline hooks). **Decision fork: want harness control → rig (#1); just want models cheaply → Option C (keep claude -p + backend swap).** Goose sits between (adopt a harness, gain some control, cede byte-exact prompt).

**Spike to validate Option C (not yet run — needs a real backend; mechanism proven, model behavior not):** `claude -p → mistral.rs (native Anthropic) or proxy→vLLM → Qwen/GLM` on RightClaw's hard schemas — does the **unforced** `StructuredOutput` tool get called reliably, and does the backend tolerate claude's beta headers + 59 tool defs? Reusable claude-side tester: `docs/spike/harness/fake_anthropic.py`.
