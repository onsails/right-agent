...
- **S2. Setup-token auth.** Inject a real per-agent Claude OAuth token as `ANTHROPIC_OAUTH_TOKEN` into `omp -p` — verify the Anthropic wire accepts it (headers/`anthropic-beta`). If it fails, the whole omp/pi path for subscription agents collapses to API-key-only → decision input.
...
1. `ANTHROPIC_OAUTH_TOKEN` + setup-token compatibility (S2) — path-destroying if negative.
