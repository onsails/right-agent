    ...
    pub async fn apply(self) -> MicrosandboxResult<SandboxModificationPlan> {
        ...
        if !restart_required {
            ...
            if !updates.is_empty() {
                ...
                // The running secret set changed: mirror it into the active
                // snapshot. Final removal preserves active TLS-on because the
                // live protocol cannot toggle interception itself.
                if let Some(active) = active.as_mut() {
                    apply_secret_patch_to_active_config(active, &self.patch)?;
                    persist_active_config(&self.backend, &handle, active).await?;
                }
            }
        }
...
fn apply_secret_patch_to_config(
    ...
    // Desired TLS interception is enabled exactly while at least one
    // substitution binding exists. The active snapshot separately preserves
    // TLS-on after final live removal because the runtime lacks a TLS toggle.
    network.tls.enabled = !network.secrets.secrets.is_empty();
...
/// Mirror a live secret patch into the active snapshot without claiming that
/// TLS interception itself was disabled live. The runtime control protocol can
/// remove every binding immediately, but it has no TLS-enable toggle; after the
/// final removal the desired config is TLS-off while the current VM remains
/// TLS-on until its next start.
#[cfg(feature = "net")]
fn apply_secret_patch_to_active_config(
    config: &mut SandboxConfig,
    patch: &SandboxModificationPatch,
) -> MicrosandboxResult<()> {
    let was_tls_enabled = config.local_network_config()?.tls.enabled;
    apply_secret_patch_to_config(config, patch)?;
    let mut network = config.local_network_config()?;
    if was_tls_enabled && network.secrets.secrets.is_empty() {
...
fn resolve_secret_source_value(
    ...
    match source {
        Some(SecretSource::Env { var }) => {
            let value = match super::config::resolve_installed_secret(var) {
                Some(value) => value.as_str().to_owned(),
                None => std::env::var(var).map_err(|_| {
                    crate::MicrosandboxError::InvalidConfig(format!(
                        "secret {name}: source {var} could not be resolved"
                    ))
                })?,
            };
            if value.is_empty() {
                return Err(crate::MicrosandboxError::InvalidConfig(format!(
                    "secret {name}: source {var} resolved to an empty value"
                )));
            }
            Ok(value)
        }
...
mod tests {
    ...
    #[cfg(feature = "net")]
    #[test]
    fn applying_secret_remove_deletes_entry_and_disables_tls() {
        let mut config = config_with_secret("API_KEY", SECRET_SENTINEL);
        let patch = SandboxModificationPatch {
            secrets_remove: vec!["API_KEY".to_string()],
            ..SandboxModificationPatch::default()
    ...
    #[cfg(feature = "net")]
    #[test]
    fn active_final_removal_keeps_tls_enabled_until_restart() {
        let mut config = config_with_secret("API_KEY", SECRET_SENTINEL);
        let patch = SandboxModificationPatch {
            secrets_remove: vec!["API_KEY".to_string()],
            ..SandboxModificationPatch::default()
        ...
        apply_secret_patch_to_active_config(&mut config, &patch).unwrap();
    ...
    #[cfg(feature = "net")]
    #[test]
    fn applying_secret_remove_keeps_tls_enabled_for_survivor() {
        let mut config = config_with_secret("API_KEY", SECRET_SENTINEL);
        let mut network = config.local_network_config().unwrap();
        let mut survivor = network.secrets.secrets[0].clone();
        survivor.env_var = "SURVIVOR_KEY".to_string();
        survivor.placeholder = "$MSB_SURVIVOR_KEY".to_string();
        network.secrets.secrets.push(survivor);
        config.set_local_network_config(network).unwrap();
        let patch = SandboxModificationPatch {
            secrets_remove: vec!["API_KEY".to_string()],
            ..SandboxModificationPatch::default()
        ...
        assert_eq!(network.secrets.secrets[0].env_var, "SURVIVOR_KEY");
        assert!(network.tls.enabled);
    }
