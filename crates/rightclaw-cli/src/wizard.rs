...
fn format_memory_display(
    memory: &Option<rightclaw::agent::types::MemoryConfig>,
    agent_name: &str,
) -> String {
    use rightclaw::agent::types::MemoryProvider;
...
async fn memory_setup(
    current: Option<&rightclaw::agent::types::MemoryConfig>,
    agent_name: &str,
) -> miette::Result<Option<rightclaw::agent::types::MemoryConfig>> {
    use rightclaw::agent::types::{MemoryConfig, MemoryProvider};
    use rightclaw::init::{
    ...
    if let Some(k) = resolved_key.as_deref() {
        ...
        match rightclaw::init::validate_hindsight_key(k).await {
...
fn update_agent_yaml_memory(
    path: &Path,
    cfg: &rightclaw::agent::types::MemoryConfig,
) -> miette::Result<()> {
    use rightclaw::agent::types::MemoryProvider;
    use rightclaw::init::{DEFAULT_RECALL_BUDGET, DEFAULT_RECALL_MAX_TOKENS};
...
mod memory_yaml_tests {
    use super::*;
    use rightclaw::agent::types::{MemoryConfig, MemoryProvider, RecallBudget};
    ...
    fn parse_agent_config(path: &std::path::Path) -> rightclaw::agent::AgentConfig {
    ...
    fn update_agent_yaml_memory_omits_default_recall_fields() {
        ...
        let cfg = MemoryConfig {
            ...
            bank_id: None,
            recall_budget: rightclaw::init::DEFAULT_RECALL_BUDGET,
            recall_max_tokens: rightclaw::init::DEFAULT_RECALL_MAX_TOKENS,
        };
        update_agent_yaml_memory(&path, &cfg).unwrap();
