...
mod tests {
    ...
    async fn gate_skips_when_daily_budget_exceeded() {
        let (_dir, conn) = conn().await;
        ensure_agent_nudge_state(&conn, "him").await;
        ...
        let decision = try_mark_review_started(&conn, "him", input).await.unwrap();
        assert_eq!(
            decision,
    ...
    async fn record_review_failure_increments_and_returns_opened_false_below_threshold() {
        ...
        let (count, opened) = record_review_failure(&conn, "him", "2026-05-21T03:00:00Z", 5, 60)
            .await
            .unwrap();
        assert_eq!(count, 3);
        assert!(!opened);
    ...
    async fn record_review_failure_rejects_invalid_now_utc() {
        let (_dir, conn) = conn().await;
        ensure_agent_nudge_state(&conn, "him").await;
        let result = record_review_failure(&conn, "him", "not-a-timestamp", 5, 60).await;
        assert!(
            result.is_err(),
