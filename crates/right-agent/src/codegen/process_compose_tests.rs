...
#[test]
fn bot_agent_command_contains_right_bot_agent() {
    let agents = vec![make_bot_agent("myagent", "123:tok")];
    let exe = Path::new(EXE_PATH);
    let output = generate_process_compose(&agents, exe, &default_config()).unwrap();
    assert!(
        output.contains("right bot --agent myagent"),
        "expected 'right bot --agent myagent' in:\n{output}"
    );
}
...
fn bot_agent_env_contains_rc_agent_dir() {
    ...
    let output = generate_process_compose(&agents, exe, &default_config()).unwrap();
    assert!(
        output.contains("RC_AGENT_DIR=/home/user/.right/agents/myagent"),
...
// ENABLE_TOOL_SEARCH and MCP_CONNECTION_NONBLOCKING were removed:
// - ENABLE_TOOL_SEARCH=false blocked ToolSearch for all tools including MCP discovery
// - MCP_CONNECTION_NONBLOCKING=1 is unnecessary — synchronous MCP connect is preferred
...
fn output_starts_with_generated_comment() {
    ...
    let output = generate_process_compose(&agents, exe, &default_config()).unwrap();
    assert!(
        output.starts_with("# Generated by right"),
        "expected '# Generated by right' at start of:\n{output}"
    );
}
...
fn cloudflared_with_script_produces_process_entry() {
    ...
    assert!(
        output.contains("command: \"/home/user/.right/scripts/cloudflared-start.sh\""),
        "expected absolute script path in cloudflared command:\n{output}"
    );
    assert!(
        output.contains("working_dir: \"/home/user/.right\""),
