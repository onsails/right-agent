...
pub async fn insert_learning_selector(
    conn: &Connection,
    b: &UsageBreakdown,
    episode_id: i64,
) -> Result<(), UsageError> {
    let job = episode_id.to_string();
    insert_row(conn, b, "learning_selector", None, None, Some(job.as_str())).await
}
...
pub async fn insert_learning_reviewer(
    conn: &Connection,
    b: &UsageBreakdown,
    episode_id: i64,
) -> Result<(), UsageError> {
    let job = episode_id.to_string();
    insert_row(conn, b, "learning_reviewer", None, None, Some(job.as_str())).await
}

/// Insert a row for a worker-side learned-skill review invocation.
pub async fn insert_learning_skill_review(
    conn: &Connection,
    b: &UsageBreakdown,
    chat_id: i64,
    thread_id: i64,
...
mod tests {
    ...
    #[tokio::test]
    async fn insert_learning_selector_writes_row_with_correct_source_and_job_name() {
        let dir = tempdir().unwrap();
        let conn = open_connection(dir.path(), true).await.unwrap();
        insert_learning_selector(&conn, &sample_breakdown(), 42)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn insert_learning_reviewer_writes_row_with_correct_source_and_job_name() {
        let dir = tempdir().unwrap();
        let conn = open_connection(dir.path(), true).await.unwrap();
        insert_learning_reviewer(&conn, &sample_breakdown(), 99)
            .await
            .unwrap();
        let (source, job_name): (String, Option<String>) = conn
            .query_row(
    ...
    #[tokio::test]
    async fn insert_learning_skill_review_writes_row_with_correct_source_chat_thread() {
        let dir = tempdir().unwrap();
        let conn = open_connection(dir.path(), true).await.unwrap();
        insert_learning_skill_review(&conn, &sample_breakdown(), -1001, 7)
            .await
            .unwrap();
