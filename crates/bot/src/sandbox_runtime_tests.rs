...
/// The only initial state a unit test can build: no sandbox can boot here, so
/// every handle starts from a bring-up failure.
fn unavailable() -> Arc<right_sandbox::SandboxDiagnosis> {
    Arc::new(SandboxCause::HypervisorUnavailable.diagnose())
}

#[test]
fn new_handle_starts_unavailable() {
    let (h, _rx) = SandboxRuntimeHandle::new(Err(unavailable()));
...
async fn report_suspected_failure_wakes_supervisor() {
    let (h, mut rx) = SandboxRuntimeHandle::new(Err(unavailable()));
...
fn unavailable_backend_replies_never_proceeds() {
    match sandbox_gate(&SandboxHealth::Unavailable {
