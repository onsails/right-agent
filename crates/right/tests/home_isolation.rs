...
fn init_agent_claude_json_has_trust() {
    ...
    let has_trust = projects
        .values()
        .any(|proj| proj.get("hasTrustDialogAccepted").and_then(|v| v.as_bool()) == Some(true));
    assert!(
        has_trust,
        ".claude.json should have hasTrustDialogAccepted: true, got: {content}"
    );
}
    ...
    if creds_path.exists() || creds_path.symlink_metadata().is_ok() {
        ...
        assert!(
            target
                .to_str()
                .unwrap()
                .contains(".claude/.credentials.json"),
...
fn init_warns_when_host_creds_missing() {
    ...
    // When host creds are missing, right should still succeed (warn, not error)
    // and stderr should contain the warning about missing credentials
    result.stderr(
        predicates::str::contains("no OAuth credentials")
            .or(predicates::str::contains("ANTHROPIC_API_KEY")),
    );
}
