...
mod provider_validation_tests {
    ...
    fn serialize_provider_entry_emits_shared_from_only_when_borrowed() {
        ...
        assert!(s.contains("shared_from: 'agent-a'"), "got: {s}");
...
mod credential_guard_tests {
    ...
    fn source_credential_readable_rejects_redaction_sentinel() {
        ...
        let err = check_source_credential_readable(REDACTION_SENTINEL, "agent-a-provider")
    ...
    fn source_credential_readable_rejects_empty() {
        assert!(check_source_credential_readable("", "agent-a-provider").is_err());
    }

    #[test]
    fn source_credential_readable_accepts_real_value() {
        check_source_credential_readable("falid-aaaa-bbbb:0123456789abcdef", "agent-a-provider")
...
// Retained as defense-in-depth: any future host-side credential read-back caller
// must reject a redacted ("REDACTED"/empty) value before writing it. See the
// canary test ci_openshell_get_provider_redacts.rs and provider-sharing plan Task 8.
#[allow(dead_code)]
