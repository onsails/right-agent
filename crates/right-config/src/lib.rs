...
/// Tunnel configuration: a public hostname plus a provider that decides who
/// terminates TLS and reverse-proxies the public hostname into per-agent
/// `bot.sock` Unix sockets.
#[derive(Debug, Clone)]
pub struct TunnelConfig {
    /// Public hostname the bot tells Telegram (`https://<hostname>/tg/<agent>`).
    pub hostname: String,
    /// Selects the runtime tunnel implementation.
    pub provider: TunnelProvider,
}

/// Tunnel implementation strategy.
///
/// - `Cloudflared` — `right up` writes the cloudflared ingress config and
///   runs cloudflared as a process-compose child. Requires `tunnel_uuid` and
///   a credentials file generated by `cloudflared tunnel create`.
/// - `External` — public TLS fronting is owned by the operator (e.g. a caddy
///   or nginx running outside process-compose). `right up` skips all
///   cloudflared codegen and never starts a cloudflared process.
...
impl TunnelProvider {
    /// True when `right` owns the tunnel process and config files.
    pub fn is_cloudflared(&self) -> bool {
        matches!(self, TunnelProvider::Cloudflared { .. })
    }
}
...
struct RawTunnelConfig {
    ...
    #[serde(default)]
    hostname: String,
    /// Selects the tunnel implementation. Defaults to `cloudflared` for
...
pub fn read_global_config(home: &Path) -> miette::Result<GlobalConfig> {
    ...
    let raw_tunnel = raw.tunnel.ok_or_else(|| {
        miette::miette!(
            help = "run: right init --tunnel-name NAME --tunnel-hostname HOSTNAME",
            "config.yaml has no `tunnel:` block — a tunnel is required (re-run `right init`)"
        )
    })?;
    if raw_tunnel.hostname.is_empty() {
        return Err(miette::miette!(
            help = "set `tunnel.hostname` in config.yaml (e.g. right.example.com)",
            "Tunnel config is missing `hostname` — the bot needs a public hostname to register the Telegram webhook"
        ));
    }
    let provider_name = raw_tunnel
        .provider
        .as_deref()
        .map(str::trim)
        .filter(|s| !s.is_empty())
    ...
    let provider = match provider_name {
        "cloudflared" => {
            if raw_tunnel.credentials_file.is_empty() || raw_tunnel.tunnel_uuid.is_empty() {
                return Err(miette::miette!(
                    help = "run: right init --tunnel-name NAME --tunnel-hostname HOSTNAME (or set `tunnel.provider: external` to bring your own reverse proxy)",
        ...
        "external" => TunnelProvider::External,
        other => {
            return Err(miette::miette!(
                help = "supported values: `cloudflared`, `external`",
                "Unknown tunnel provider `{other}` in config.yaml"
            ));
        }
    };
    Ok(GlobalConfig {
...
pub fn write_global_config(home: &Path, config: &GlobalConfig) -> miette::Result<()> {
    ...
    match &config.tunnel.provider {
        TunnelProvider::Cloudflared {
            tunnel_uuid,
            credentials_file,
        } => {
            let uuid = tunnel_uuid.replace('"', "\\\"");
            let creds = credentials_file.display().to_string().replace('"', "\\\"");
            content.push_str("  provider: \"cloudflared\"\n");
            content.push_str(&format!("  tunnel_uuid: \"{uuid}\"\n"));
            content.push_str(&format!("  credentials_file: \"{creds}\"\n"));
        }
        TunnelProvider::External => {
            content.push_str("  provider: \"external\"\n");
        }
    }
...
mod tests {
    ...
    fn write_then_read_roundtrips_new_fields() {
        ...
        match read.tunnel.provider {
            TunnelProvider::Cloudflared {
                tunnel_uuid,
                credentials_file,
            } => {
                assert_eq!(tunnel_uuid, "abc-123");
                assert_eq!(credentials_file, PathBuf::from("/tmp/abc-123.json"));
            }
            other => panic!("expected Cloudflared provider, got {other:?}"),
        }
    }
    ...
    #[test]
    fn legacy_config_without_provider_field_parses_as_cloudflared() {
        let dir = TempDir::new().unwrap();
        let yaml = "tunnel:\n  tunnel_uuid: \"u\"\n  credentials_file: \"/x\"\n  hostname: \"h.example.com\"\n";
        std::fs::write(dir.path().join("config.yaml"), yaml).unwrap();
        let config = read_global_config(dir.path()).unwrap();
        assert_eq!(config.tunnel.hostname, "h.example.com");
        assert!(
            config.tunnel.provider.is_cloudflared(),
            "missing provider field must default to cloudflared for backward compat"
        );
    }

    #[test]
    fn external_provider_needs_only_hostname() {
        let dir = TempDir::new().unwrap();
        let yaml = "tunnel:\n  provider: \"external\"\n  hostname: \"bot.example.com\"\n";
        std::fs::write(dir.path().join("config.yaml"), yaml).unwrap();
        let config = read_global_config(dir.path()).unwrap();
        assert_eq!(config.tunnel.hostname, "bot.example.com");
        assert_eq!(config.tunnel.provider, TunnelProvider::External);
    }

    #[test]
    fn external_provider_roundtrips() {
        let dir = TempDir::new().unwrap();
        let written = GlobalConfig {
            tunnel: TunnelConfig {
                hostname: "bot.example.com".to_string(),
                provider: TunnelProvider::External,
            },
            aggregator: AggregatorConfig::default(),
        };
        write_global_config(dir.path(), &written).unwrap();
        let content = std::fs::read_to_string(dir.path().join("config.yaml")).unwrap();
        assert!(
            content.contains("provider: \"external\""),
            "written YAML must declare external provider, got: {content}"
        );
        assert!(
            !content.contains("tunnel_uuid"),
            "external provider must not emit tunnel_uuid, got: {content}"
        );
        assert!(
            !content.contains("credentials_file"),
            "external provider must not emit credentials_file, got: {content}"
        );
        let read = read_global_config(dir.path()).unwrap();
        assert_eq!(read.tunnel.provider, TunnelProvider::External);
    }

    #[test]
    fn missing_hostname_yields_error() {
        let dir = TempDir::new().unwrap();
        let yaml = "tunnel:\n  provider: \"external\"\n";
        std::fs::write(dir.path().join("config.yaml"), yaml).unwrap();
        let err = read_global_config(dir.path()).unwrap_err();
        assert!(
            format!("{err:#}").contains("hostname"),
            "error must mention hostname, got: {err}"
        );
    }

    #[test]
    fn unknown_provider_yields_error() {
        let dir = TempDir::new().unwrap();
        let yaml = "tunnel:\n  provider: \"ngrok\"\n  hostname: \"h.example.com\"\n";
        std::fs::write(dir.path().join("config.yaml"), yaml).unwrap();
        let err = read_global_config(dir.path()).unwrap_err();
        let msg = format!("{err:#}");
        assert!(
            msg.contains("ngrok") && msg.contains("provider"),
    ...
    fn aggregator_allowed_hosts_roundtrip() {
        ...
        assert!(read.tunnel.provider.is_cloudflared());
    }
