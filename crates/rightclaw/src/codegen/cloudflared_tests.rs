...
fn https_scheme_stripped_from_hostname() {
    ...
    let yaml =
        generate_cloudflared_config(&agents, "https://right.example.com", None).unwrap();
    assert!(
...
fn http_scheme_stripped_from_hostname() {
    let agents = vec![("agent".to_string(), PathBuf::from("/tmp/agents/agent"))];
    let yaml =
        generate_cloudflared_config(&agents, "http://right.example.com", None).unwrap();
    assert!(
        yaml.contains("hostname: right.example.com"),
...
fn credentials_embedded_when_provided() {
    ...
    let yaml =
        generate_cloudflared_config(&agents, "right.example.com", Some(&creds)).unwrap();
    assert!(
    ...
    assert!(
        yaml.contains(
            "credentials-file: /home/user/.rightclaw/tunnel/e765cc71-d0c2-42a3-864b-81566f8817fd.json"
        ),
