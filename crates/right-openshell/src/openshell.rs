    ...
    if exists && !force_recreate {
        return Err(miette::miette!(
            help = "Use --force to recreate the sandbox,\n  \
                    or `right agent config` to update an existing agent",
...
/// Wait until a generic provider's composed rule has the exact desired
/// endpoint host/path set. Use this for config-confirmation paths where stale
/// extra endpoints would leave removed hosts active.
pub async fn wait_for_provider_composed_with_exact_endpoints(
    client: &mut OpenShellClient<Channel>,
    sandbox_name: &str,
    provider_name: &str,
    expected: Vec<(String, String)>,
    ...
    wait_for_provider_composed_where(
        ...
        sandbox_name,
        provider_name,
        format!("exact endpoints {expected_label}"),
        |policy| {
