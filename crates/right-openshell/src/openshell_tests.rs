...
use crate::openshell_proto::openshell::v1::{SandboxCondition, SandboxStatus};
...
async fn wait_for_provider_composed_tolerates_transient_read_errors() {
    ...
    let mock = MockOpenShell {
        mock_get_sandbox_policy_status: Some(Box::new(move |_req| {
            ...
            policy.network_policies.insert(
                "_provider_right_example".to_string(),
                NetworkPolicyRule {
                    name: "_provider_right_example".to_string(),
                    ..Default::default()
                },
            );
    ...
    wait_for_provider_composed(&mut client, "sbx", "right-example")
        .await
        .expect("transient read error must not abort the composition wait");
    assert!(
        calls.load(Ordering::SeqCst) >= 2,
        "wait should have polled again after the transient read error"
    );
}
